...
export function AIContentStudio() {
    ...
    const handleUpload = async (blob: Blob) => {
        const toastId = toast.loading("Uploading to Decentralized Storage...");
        try {
            const url = await storageService.upload(blob, `studio/${Date.now()}`);
    ...
    return (
        <div className="h-full bg-black text-white flex flex-col relative">
             {isLive && <LiveStreamView onClose={() => setIsLive(false)} />}
             ...
             <div className="flex-1 overflow-y-auto p-4 relative">
                 <AnimatePresence mode="wait">
                     ...
                     {mode === 'record' && (
                         ...
                         >
                             <div className="w-full max-w-sm aspect-[9/16] bg-black rounded-2xl relative overflow-hidden shadow-2xl border-2 border-gray-700">
                                 <CameraRecorder script={script} onUpload={handleUpload} />
                             </div>
                         </motion.div>
                     )}
                     ...
                     {mode === 'live' && (
                         ...
                         >
                             ...
                             <button
                                ...
                                className="px-10 py-4 bg-gradient-to-r from-blue-600 to-cyan-500 rounded-full font-bold text-white shadow-xl shadow-blue-500/20 hover:scale-105 transition-transform flex items-center gap-2"
                             >
