...
export function parseCommand(input: string, context: string = 'home'): { response: string, action: AIAction } {
    ...
    if (lower.includes("draw") || lower.includes("generate") || lower.includes("paint") || lower.includes("image of") || lower.includes("picture of")) {
        ...
        return {
            response: `Manifesting visual data for "${prompt}" using Generative Adversarial Networks...`,
            action: {
    ...
    // Default
    return { response: "I am online. Command me to Generate Images, Search, Navigate, or Analyze Data.", action: { type: 'NONE' } };
}
