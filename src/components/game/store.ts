...
export interface GameState {
  ...
  breedPets: (sireId: string, damId: string) => Pet;
...
export const useGameStore = create<GameState>()(
  persist(
    (set, get) => ({
      ...
      stats: { totalBreeds: 0, totalWins: 0, itemsBought: 0 },

      addPi: (amount) => set((state) => ({ piBalance: state.piBalance + amount })),

      buyItem: (itemId, cost) => {
        const { piBalance, inventory, stats } = get();
        if (piBalance >= cost) {
          set({
            ...
            stats: { ...stats, itemsBought: (stats?.itemsBought || 0) + 1 }
          });
          return true;
        }
        return false;
      ...
      breedPets: (sireId, damId) => {
        const state = get();
        if (state.piBalance < BREEDING_FEE) throw new Error("Not enough Pi");

        const sire = state.pets.find(p => p.id === sireId);
        const dam = state.pets.find(p => p.id === damId);
        if (!sire || !dam) throw new Error("Parents not found");

        const COOLDOWN = 3600 * 1000;
        if (Date.now() - (sire.lastBred || 0) < COOLDOWN) throw new Error(`${sire.name} needs to rest`);
        if (Date.now() - (dam.lastBred || 0) < COOLDOWN) throw new Error(`${dam.name} needs to rest`);
        ...
        let childElement = sire.element;
        if (rand > 0.4) childElement = dam.element;
        if (rand > 0.8) childElement = ELEMENTS[Math.floor(Math.random() * ELEMENTS.length)];

        parts.forEach(part => {
            const parentGene = Math.random() < 0.5 ? sire.genes[part] : dam.genes[part];

            if (Math.random() < 0.1) {
                genes[part] = generateRandomGene(part, childElement, 0.2);
            } else {
                genes[part] = { ...parentGene };
            }

            totalStats.str += genes[part].statsBonus.str;
            totalStats.int += genes[part].statsBonus.int;
            totalStats.spd += genes[part].statsBonus.spd;
            totalStats.vit += genes[part].statsBonus.vit;
        });

        const newPet: Pet = {
            id: `pet_bred_${Date.now()}`,
            name: `Offspring of ${sire.name}`,
        ...
        const updatedPets = state.pets.map(p => {
            if (p.id === sire.id || p.id === dam.id) {
                return { ...p, lastBred: Date.now() };
            }
            return p;
        });
        ...
        set({
            piBalance: state.piBalance - BREEDING_FEE,
            pets: [newPet, ...updatedPets],
            stats: { ...state.stats, totalBreeds: (state.stats?.totalBreeds || 0) + 1 }
        });
      ...
      recordBattleResult: (petId, win, reward) => {
          set(state => ({
              ...
              stats: { ...state.stats, totalWins: (state.stats?.totalWins || 0) + (win ? 1 : 0) },
