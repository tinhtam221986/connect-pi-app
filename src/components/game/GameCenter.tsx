...
import { Hammer, Trophy, Zap, Coins, Gamepad2, Dna } from "lucide-react";
...
export function GameCenter() {
    const { t } = useLanguage();
    const { balance, addToBalance } = useEconomy();
    const [score, setScore] = useState(0);
    const [showGeneLab, setShowGeneLab] = useState(false);
        ...
        <div className="h-full bg-black text-white flex flex-col pb-20 overflow-y-auto">
            ...
            <div className="p-4 border-b border-gray-800 bg-gray-900/50 backdrop-blur-md sticky top-0 z-10 flex justify-between items-center">
                 ...
                 <div className="bg-gray-800 px-3 py-1 rounded-full flex items-center gap-2 border border-yellow-500/30">
                     ...
                     <span className="font-bold text-yellow-400">{balance.toFixed(0)} Pi</span>
                 </div>
                 <button onClick={() => setShowGeneLab(true)} className="ml-2 p-2 bg-purple-600 rounded-full text-white shadow-lg hover:bg-purple-500">
                     <Dna size={20} />
                 </button>
            </div>

            {showGeneLab && <GeneLab onClose={() => setShowGeneLab(false)} />}
