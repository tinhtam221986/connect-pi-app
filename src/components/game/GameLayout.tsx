...
import { ShoppingCart, FlaskConical, Cat, Coins, ArrowLeft } from "lucide-react";
...
export function GameLayout({ onBack }: GameLayoutProps) {
  const [tab, setTab] = useState<'shop' | 'lab' | 'collection'>('shop');
    ...
    <div className="flex flex-col h-full bg-slate-950 text-white animate-in fade-in duration-300">
      ...
      <div className="flex border-t border-slate-800 bg-slate-900 pb-safe">
        ...
        >
            ...
            <span className="text-[10px] font-bold uppercase">Cửa Hàng</span>
        </button>
        <button
            onClick={() => setTab('lab')}
        ...
        >
            ...
            <span className="text-[10px] font-bold uppercase">Phòng Lab</span>
        </button>
        <button
        ...
        >
            <Cat size={24} />
            <span className="text-[10px] font-bold uppercase">Thú Cưng</span>
        </button>
      </div>
    </div>
  );
