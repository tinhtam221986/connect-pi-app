...
import { Heart, MessageCircle, Share2, Music2, Disc, Gift, Loader2 } from "lucide-react";
...
export function VideoFeed() {
  ...
  const fetchVideos = async () => {
      setLoading(true);
      const data = await apiClient.feed.get(language);
      setVideos(data);
      setLoading(false);
  };
  ...
  return (
    <div className="h-full w-full relative bg-black">
        ...
        <button onClick={fetchVideos} className="absolute top-4 right-4 z-20 text-xs text-gray-500 bg-black/20 p-1 rounded hover:bg-black/50">Refresh</button>
